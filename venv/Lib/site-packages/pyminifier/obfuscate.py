...
if not isinstance(sys.version_info, tuple):
    if sys.version_info.major == 3:
        chr = chr # So we can support both 2 and 3
...
def obfuscatable_variable(tokens, index, ignore_length=False):
    ...
    if token_string == "for":
        return None
    if token_string in list(keyword_args.keys()):
        return None
...
def remap_name(name_generator, names, table=None):
    ...
    out = ""
    for name in names:
        if table and name in list(table[0].keys()):
...
def obfuscate_builtins(module, tokens, name_generator, table=None):
    ...
    replacement_dict = dict(list(zip(used_builtins, replacements)))
    if table:
    ...
    *module* must be the name of the module we're currently obfuscating
    ...
    if table:
        ...
        for module_method in module_methods:
            if module_method in list(table[0].keys()):
        ...
        replacement_dict = dict(list(zip(module_methods, method_map)))
    import_line = False
    ...
    for module_method, replacement in list(replacement_dict.items()):
        indents = []
        index = 0
        for tok in tokens[:]:
            token_type = tok[0]
